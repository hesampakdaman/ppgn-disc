...
ARGS.ckpts = '../ckpts/vanilla'
ARGS.samplepth = '../samples/vanilla'
ARGS.plotspth = '../plots/plot_vanilla'
...
def sampler(e1, e2, e3, iters, cols, filename='samples'):
    ...
    sys.stdout.write('\r[vanilla] [sample] [0/%d]' %(N))
    ...
    for c in range(classes):
        for _ in range(cols):
            ...
            sys.stdout.write('\r[vanilla] [sample] [%d/%d]' %(i,N))
            sys.stdout.flush()
...
if(ARGS.sample):
    sampler(1e-2, 1e0, 1e-15, 200, 10, 'vanilla')
    print('')
    sys.exit(1)
