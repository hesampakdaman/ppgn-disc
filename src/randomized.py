...
ARGS.ckpts = '../ckpts/randomized'
ARGS.samplepth = '../samples/randomized'
ARGS.plotspth = '../plots/plot_randomized'
...
def sampler(e1, e2, e3, iters, cols, filename='samples'):
    ...
    sys.stdout.write('\r[randomized] [sample] [0/%d]' %(N))
    ...
    for c in range(classes):
        for _ in range(cols):
            ...
            sys.stdout.write('\r[randomized] [sample] [%d/%d]' %(i,N))
            sys.stdout.flush()
...
if(ARGS.sample):
    sampler(1e-2, 1e0, 1e-15, 200, 10, 'randomized')
    print('')
    sys.exit(1)
...
# initialized in case not
# calculated in time for printing
Gx_cost = 0
Gh_cost = 0
Dx_cost = 0
Dh_cost = 0
...
for epoch in range(1, EPOCH_MAX+1):
    ...
    while i < datalen:
        ...
        while j < Diters and i < len(dataset):
            ...
            rand = np.random.uniform() # uniformly in [0,1)
            probX = 0.5

            if(rand < probX or G_step < 25 or G_step % 500 == 0):
                ###########################
                # Discriminate images
                ###########################
                Dx_trained = True
                ...
                gradient_penalty = calc_gradient_penalty(netDx,
                                                        real_data_v.data,
                                                        fake.data)
                gradient_penalty.backward()

                Dx_cost = Dx_fake.data[0] - Dx_real.data[0]\
            ...
            if(rand >= probX or G_step < 25 or G_step % 500 == 0):
                ...
                gradient_penalty = calc_gradient_penalty(netDh,
                                                        hv.data, fake.data)
                gradient_penalty.backward()

                Dh_cost = Dh_fake.data[0] - Dh_real.data[0]\
                        + gradient_penalty.data[0]
        ...
        if(Dx_trained or G_step < 25 or G_step % 500 == 0):
            # L_gan_x
            hv = Variable(h)
            fake = netG(hv)
            G = netDx(fake)
        ...
        if(not Dx_trained or G_step < 25 or G_step % 500 == 0):
            # L_gan_h
            hv = Variable(h)
            x_fake = netG(hv)
            fake = netE(x_fake.view(-1, 1, 28, 28))[ARGS.Dhlayer]
        ...
        sys.stdout.write(
                '\r[randomized] [Epoch: %d/%d] [%d] [%d:%d] '\
                '[L_D_x: %f, L_D_h: %f] '\
                '[L_gan_x: %f L_gan_h: %f L_x: %f  L_h: %f]'\
                %(
                    epoch, EPOCH_MAX, G_step, Dx_step, Dh_step,
                    Dx_cost, Dh_cost, Gx_cost,
